...
class Router(object):
    ...
    def __get_route(self, dynamicforwardingtable, forwardtable, dst):

        rt = None
        name = None
        max_prefixnet = None

        for entry in dynamicforwardingtable:
    ...
    def router_main(self, net):
        ...
        arp_map = {}
        dynamic_routing_table = deque(maxlen = 5)
            ...
            if gotpkt:
                ...
                if pkt.has_header(Arp):
                    ...
                    if arp.operation is ArpOperation.Reply and str(arp.targetprotoaddr) in my_IP_Eth:
                        ...
                        replyPkt = create_ip_arp_reply(my_IP_Eth[str(arp.targetprotoaddr)], \
                ...
                if pkt.has_header(DynamicRoutingMessage):
                    ...
                    log_debug("Got a Dynamic Routing Message {}".format(str(drm)))

                    entry = tableEntry(drm.advertised_prefix, drm.advertised_mask, \
                                       drm.next_hop, dev)

                    dynamic_routing_table.append(entry)
                ...
                if pkt.has_header(IPv4):
                    ...
                    route, srcName = self.__get_route(dynamic_routing_table, forwardtable, ipv4.dst)
                ...
                for entry in pkt_list:
                    route, srcName = self.__get_route(dynamic_routing_table, forwardtable, ipv4.dst)
                    srctemp = self.__get_intf(my_interfaces, srcName)  # sender
                    log_debug("searching for {} ".format(route))
